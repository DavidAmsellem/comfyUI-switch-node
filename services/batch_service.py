...
def cancel_batch_job(batch_id):
    ...
    # 2. Acciones a ComfyUI (Fuera del lock)
    if pids_to_kill:
        interrupt_current_processing()
        delete_queue_items(pids_to_kill)
    
    # 3. LIMPIEZA DE ARCHIVOS FÍSICOS
    ...
    # 4. Limpiar referencias en memoria para que no salgan más en el frontend
    with BATCH_LOCK:
        ACTIVE_BATCHES[batch_id]['results'] = [] # Vaciamos la lista
        ACTIVE_BATCHES[batch_id]['completed_workflows'] = 0 

    # 5. Actualizar persistencia (JSON)
